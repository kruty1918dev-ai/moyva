...
const systemPrompt = [
  ...
  '- Шлях файлу ОБОВЯЗКОВО має починатися з docs/',
].join('\n');
...
// 1. Validate token
const modelsListRes = await fetchFn(apiBase + '/models', {
  method: 'GET',
  headers: {
...
const completionRes = await fetchFn(apiBase + '/chat/completions', {
  method: 'POST',
  headers: {
