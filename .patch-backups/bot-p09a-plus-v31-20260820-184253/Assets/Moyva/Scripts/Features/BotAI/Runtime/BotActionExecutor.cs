    ...
    internal sealed class BotActionExecutor : IBotActionExecutor
    {
            ...
            CancellationToken token)
        {
            ...
            return action.Kind switch
            {
                ...
                BotActionKind.Recruit => ExecuteRecruit(ownerId, action),
        ...
        private BotActionExecutionResult ExecuteRecruit(string ownerId, BotActionCandidate action)
        {
            if (_recruitment == null)
                return BotActionExecutionResult.Rejected("Recruitment service unavailable.");
            if (!action.TargetCell.HasValue || string.IsNullOrWhiteSpace(action.DefinitionId))
                return BotActionExecutionResult.Rejected("Recruit action is missing recruiting building or unit type.");

            bool queued = _recruitment.TryEnqueue(
                ownerId,
                action.TargetCell.Value,
                action.DefinitionId,
                out string reason);
            return queued
                ? BotActionExecutionResult.Success("Recruitment queued.")
                : BotActionExecutionResult.Rejected(reason ?? "Recruitment enqueue failed.");
        }
