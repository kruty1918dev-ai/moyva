    ...
    else
    {
            ...
            && !string.IsNullOrWhiteSpace(_selectedUnitId))
        {
            UnitMovementDiagnostics.Warn(
                trace,
                "PRIMARY_EMPTY_NO_MOVE_ROUTE",
                $"pos={signal.Position}; selected={_selectedUnitId}; " +
...
private void HandleSecondaryTileClick(Vector2Int position)
{
    ...
    bool canCommand = CanCommandUnit(_selectedUnitId);
    bool validTile = _gridService.TryGetTileData(position, out _);

    if (!canCommand || !validTile)
    {
        UnitMovementDiagnostics.Warn(
            trace,
            "SECONDARY_MOVE_REJECT",
            $"pos={position}; selected={UnitMovementDiagnostics.Safe(_selectedUnitId)}; " +
            $"canCommand={canCommand}; validTile={validTile}; " +
            $"mode={_currentMode}; localOwner={UnitMovementDiagnostics.Safe(GetLocalOwnerId())}");
        return;
    }
    ...
    if (!string.IsNullOrWhiteSpace(occupantId))
    {
        UnitMovementDiagnostics.Warn(
            trace,
            "SECONDARY_MOVE_REJECT_OCCUPIED",
            $"pos={position}; occupant={occupantId}; selected={_selectedUnitId}");
        return;
    }

    UnitMovementDiagnostics.Log(
        trace,
        "SECONDARY_MOVE_ACCEPT",
