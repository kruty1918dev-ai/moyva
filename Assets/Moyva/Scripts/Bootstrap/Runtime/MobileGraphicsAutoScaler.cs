...
namespace Kruty1918.Moyva.Bootstrap.Runtime
{
    internal sealed class MobileGraphicsAutoScaler : IInitializable, ITickable, IDisposable
    {
        ...
        private const float HealthyFpsThreshold = 58.5f;
        private const float ScaleStep = 0.05f;
        private const float AdjustmentCooldownSeconds = 4f;
        private const int HealthySamplesBeforeUpscale = 4;
        ...
        public void Tick()
        {
            ...
            {
                _healthySamples = 0;
                SetPerformanceRenderScale(_performanceRenderScale - ScaleStep, $"avgFps={_averageFps:0.0} below {LowFpsThreshold:0.0}");
                return;
            }

            if (_averageFps >= HealthyFpsThreshold)
            {
                _healthySamples++;
        ...
        {
            var nextScale = Mathf.Clamp(requestedScale, _budget.MinimumRenderScale, _budget.MaximumRenderScale);
        ...
        private static void ApplyFrameSettings(GraphicsSettingsData settings)
        {
            ...
            Application.targetFrameRate = settings.VSync ? -1 : settings.TargetFrameRate;
            OnDemandRendering.renderFrameInterval = 1;
        }
        ...
        private void SetRenderScale(float requestedScale, string reason, bool log, bool startCooldown)
        {
            ...
            if (startCooldown)
                _adjustmentCooldown = AdjustmentCooldownSeconds;
