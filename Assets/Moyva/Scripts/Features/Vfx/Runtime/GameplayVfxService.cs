        ...
        : IVfxService, IInitializable, ITickable, IDisposable
    {
        ...
        private bool PassesCameraGate(VfxEffectRule rule, Vector3 position)
        {
            ...
            if (rule.priority == 0
                ...
                && _camera.orthographicSize > _quality.LowPriorityMaxOrthoSize)
                return false;

            if (rule.maxOrthoSize > 0f
                && _camera.orthographic
                && _camera.orthographicSize > rule.maxOrthoSize)
                return false;
            ...
            if (cull > 0f)
            {
                float sqr = (_camera.transform.position - position).sqrMagnitude;
                if (sqr > cull * cull)
                    return false;
            }
