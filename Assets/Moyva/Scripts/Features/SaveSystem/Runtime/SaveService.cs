...
namespace Kruty1918.Moyva.SaveSystem
{
    internal sealed class SaveService : ISaveService, IInitializable, IDisposable
    {
        ...
        private void ExecuteLoad(byte[] bytes, int slot)
        {
            ...
            {
                Debug.LogError($"[SaveSystem] Decode failed for slot {slot}: {error}");
        ...
        private void TryLoadBackup(int slot)
        {
            ...
            {
                Debug.LogError($"[SaveSystem] No .bak available for slot {slot}. Load failed.");
                return;
            }
        ...
        private static bool ValidateSlot(int slot)
        {
            ...
            Debug.LogError($"[SaveSystem] Invalid slot={slot}. Must be 0–{MaxSlots}.");
            return false;
        }
