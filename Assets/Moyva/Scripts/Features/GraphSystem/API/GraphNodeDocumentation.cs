...
namespace Kruty1918.Moyva.GraphSystem.API
{
    ...
    public static class GraphNodeDocumentation
    {
        ...
        private static Dictionary<string, NodeDoc> BuildDocs()
        {
            ...
            Add("SeedNode", "Глобальний seed графа.", "Коли треба зробити результат генерації відтворюваним.", "Runner читає це значення до виконання шарів; нода є глобальним налаштуванням і не потребує портів або зв'язку з Output.", "Однаковий seed + однакові параметри = однаковий результат. Runtime override має пріоритет над значенням графа.", new Dictionary<string, string> { { "seed", "Глобальне зерно генерації. Значення 0 нормалізується до 1." } });
