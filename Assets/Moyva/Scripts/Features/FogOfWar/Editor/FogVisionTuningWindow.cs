...
{
    public sealed class FogVisionTuningWindow : EditorWindow
    {
        ...
        private PreviewResult ComputePreview()
        {
            float elevationStep = GetSettingFloat("ElevationStep", 0.15f, 0.01f);
            int maxVisionRange = Mathf.Max(1, GetSettingInt("MaxVisionRange", 12, 1));

            int observerBonus = Mathf.Min(
                Mathf.FloorToInt(_previewObserverHeight / elevationStep) * GetSettingInt("ObserverHeightBonusPerStep", 1, 0),
                GetSettingInt("MaxObserverHeightBonus", 4, 0));

            float downDelta = _previewObserverHeight - _previewTargetHeight;
            int downhillBonus = downDelta > 0f
                ? Mathf.Min(
                    Mathf.CeilToInt(downDelta / elevationStep) * GetSettingInt("DownhillVisionBonusPerStep", 1, 0),
                    GetSettingInt("MaxDownhillVisionBonus", 2, 0))
                : 0;

            float upDelta = _previewTargetHeight - _previewObserverHeight;
            int uphillPenalty = upDelta > 0f
                ? Mathf.Min(
                    Mathf.CeilToInt(upDelta / elevationStep) * GetSettingInt("UphillVisionPenaltyPerStep", 1, 0),
                    GetSettingInt("MaxUphillVisionPenalty", 6, 0))
                : 0;

            int effectiveRange = Mathf.Clamp(_previewBaseRange + observerBonus + downhillBonus - uphillPenalty, 0, maxVisionRange);
            bool inRange = _previewDistance <= effectiveRange;

            bool occluded = false;
            if (_previewUseBlocker && _previewDistance > 1)
            {
                float targetSlope = (_previewTargetHeight - _previewObserverHeight) / _previewDistance;
                float sampleSlope = (_previewBlockerHeight - _previewObserverHeight) / Mathf.Max(1, _previewBlockerDistance);
                float bias = GetSettingFloat("OcclusionSlopeBias", 0.02f, 0f);
                occluded = sampleSlope > targetSlope + bias;
            }

            return new PreviewResult(observerBonus, downhillBonus, uphillPenalty, effectiveRange, inRange, occluded);
        }
        ...
        {
            public readonly int ObserverBonus;
            public readonly int DownhillBonus;
            public readonly int UphillPenalty;
            public readonly int EffectiveRange;
