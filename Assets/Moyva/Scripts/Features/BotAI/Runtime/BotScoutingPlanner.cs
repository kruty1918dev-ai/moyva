    ...
    internal sealed class BotScoutingPlanner : IBotScoutingPlanner
    {
            ...
            BotStrategicContext strategy)
        {
            ...
            if (snapshot.VisibleEnemyUnits.Count > 0 &&
                strategy.Posture != BotStrategicPosture.Search)
            {
                return Array.Empty<BotActionCandidate>();
            }

            IFogStateReader fallbackFog = null;
            if (_perception == null)
            {
                if (_fogRegistry == null ||
                    !_fogRegistry.TryGetFor(
                        snapshot.OwnerId,
                        out IFogOfWarService fog) ||
                    fog == null)
                {
                    return Array.Empty<BotActionCandidate>();
                }
            ...
            var units = new List<BotUnitSnapshot>(
                snapshot.OwnUnits);
            ...
            for (int u = 0;
                 u < units.Count &&
                 scoutCount < _profile.MaxScoutsPerTurn;
                 u++)
            {
                ...
                BotUnitTacticalRole role =
                    _roles?.Resolve(config)
                    ?? BotAdvancedHeuristics.InferRole(config);

                if (role == BotUnitTacticalRole.Worker ||
                    role == BotUnitTacticalRole.Siege)
                {
                    continue;
                }

                IReadOnlyList<UnitMovementTileSnapshot> tiles =
                    _movement.GetMovementTiles(unit.UnitId);
                ...
                for (int i = 0;
                     i < tiles.Count &&
                     i < _profile.MaxTacticalTilesPerUnit;
                     i++)
                {
                    UnitMovementTileSnapshot tile = tiles[i];

                    if (!tile.IsReachable ||
                        tile.Position == unit.Position ||
                        reservedCells.Contains(tile.Position))
                    {
                        continue;
                    }

                    int sector =
                        BotAdvancedHeuristics.StableSectorKey(
                            tile.Position);

                    if (reservedSectors.Contains(sector))
                        continue;

                    int frontier = CountFrontier(
                        snapshot.OwnerId,
                        fallbackFog,
                        tile.Position,
                        2);
                    ...
                    int score =
                        450 +
                        frontier * _profile.ScoutFrontierWeight -
                        influence.Threat *
                        _profile.ScoutThreatPenaltyPercent / 100;

                    score -= Mathf.RoundToInt(
                        Mathf.Max(0f, tile.Cost) * 5f);
                ...
                reservedSectors.Add(
                    BotAdvancedHeuristics.StableSectorKey(best));
                ...
                result.Add(new BotActionCandidate(
                    ...
                    new BotActionScore(
                        bestScore,
                        "Per-bot exploration frontier reveal with local sector reservation."),
                    actorId: unit.UnitId,
                    targetCell: best,
            ...
            int radius)
        {
            ...
            for (int dx = -radius; dx <= radius; dx++)
            {
                ...
                {
                    Vector2Int p =
                        center + new Vector2Int(dx, dy);

                    if (!_grid.ContainsCell(p))
                        continue;

                    bool explored = _perception != null
                        ? _perception.IsExplored(ownerId, p)
                        : fallbackFog != null &&
                          fallbackFog.IsExplored(p);
        ...
        private int CompareScoutPreference(
            BotUnitSnapshot left,
            BotUnitSnapshot right)
        {
            UnitClassConfig lc =
                BotTacticalAnalysis.SafeGetConfig(
                    _configs,
            ...
            BotUnitTacticalRole lr =
                _roles?.Resolve(lc)
                ?? BotAdvancedHeuristics.InferRole(lc);

            BotUnitTacticalRole rr =
                _roles?.Resolve(rc)
                ?? BotAdvancedHeuristics.InferRole(rc);

            int lp =
                lr == BotUnitTacticalRole.FastScout
                    ? 0
                    : lr == BotUnitTacticalRole.Ranged
                        ? 1
                        : 2;

            int rp =
                rr == BotUnitTacticalRole.FastScout
                    ? 0
                    : rr == BotUnitTacticalRole.Ranged
                        ? 1
                        : 2;
