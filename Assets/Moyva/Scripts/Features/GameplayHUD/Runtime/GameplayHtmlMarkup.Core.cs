    ...
    internal static partial class GameplayHtmlMarkup
    {
        ...
        private static void AppendTopBar(StringBuilder html, GameplayHtmlSnapshot snapshot, GameplayHtmlState state)
        {
            html.Append("<view className=\"top-section kingdom-summary\"><view className=\"brand-mark\"></view><view className=\"stack\"><text className=\"eyebrow\">KINGDOM</text><text className=\"title\">")
                .Append(E(snapshot.KingdomName)).Append("</text></view><view className=\"resources\" data-tooltip=\"Kingdom-wide totals — construction spends local settlement stock\">");
            ...
            if (snapshot.TurnUiEnabled)
            {
                ...
                if (botMatch)
                    TurnPill(html, "VS BOT", "MODE", "local-turn");
                ...
                TurnPill(html, botMatch ? "BOT" : Display(snapshot.ActiveOwnerId), botMatch ? "OPPONENT" : "ACTIVE PLAYER", string.Empty);
                TurnPill(html, botMatch ? (snapshot.IsLocalTurn ? "YOUR TURN" : "BOT TURN") : snapshot.IsLocalTurn ? "YOUR TURN" :
                    Kruty1918.Moyva.SaveSystem.GameLaunchContext.GetPlayerController(snapshot.ActiveOwnerId)
                        == Kruty1918.Moyva.SaveSystem.PlayerControllerType.Bot ? "BOT TURN" : "WAITING",
                    "STATUS", snapshot.IsLocalTurn ? "local-turn" : "waiting-turn");
            }
            else
            {
            ...
            if (snapshot.RequiresFirstCastle || snapshot.PendingPlacementCount > 0)
            {
                ...
                if (snapshot.HasPendingSupplyDeficit)
                    html.Append(Button("SUPPLY",
                        ...
                        "button primary",
                        "Send missing resources to this settlement by wagon"));
                if (!snapshot.RequiresFirstCastle)
            ...
            else
            {
                ...
                if (snapshot.TurnUiEnabled)
                    html.Append(Button(Kruty1918.Moyva.SaveSystem.GameLaunchContext.HasBotOpponent
                            ? (snapshot.IsLocalTurn ? "END TURN" : "BOT TURN")
                            : snapshot.EndTurnPending ? "WAITING..." : "END TURN",
            ...
            GameplayHtmlState state)
        {
            ...
            Tab(html, state, KingdomDashboardTab.Logistics, "LOGISTICS", "ShowLogistics");
