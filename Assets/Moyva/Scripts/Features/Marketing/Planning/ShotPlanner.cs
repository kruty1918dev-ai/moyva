...
namespace Kruty1918.Moyva.Marketing.Planning
{
    ...
    public sealed class ShotPlanner
    {
            ...
            int seed)
        {
            ...
            {
                var subject = PickSubject(world, index, ShotCategory.Settlement, usage, rng, closeUp: false);
                if (subject.contentId == "world")
                    subject = PickSubject(world, index, ShotCategory.WorldBeauty, usage, rng, closeUp: false);
                // Vista framing: a lone tile/building (radius ~2-4) reads as a
                // dot from a reveal rig; ~10-16 shows surrounding terrain.
                subject.approximateRadius = Mathf.Clamp(subject.approximateRadius, 10f, 16f);
                sequence.shots.Add(new ShotPlan
                {
                    shotId = "loop-01",
                    category = ShotCategory.WorldBeauty,
                    rig = CameraRigType.WorldReveal,
