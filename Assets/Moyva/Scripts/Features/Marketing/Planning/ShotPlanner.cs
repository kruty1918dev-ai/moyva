...
namespace Kruty1918.Moyva.Marketing.Planning
{
    ...
    public sealed class ShotPlanner
    {
        ...
        /// <summary>Menu-background ambient footage: no beats, text or end
        /// card. shotCount ≤ 1 → a single continuous 360° orbit that wraps
        /// seamlessly when the VideoPlayer loops; otherwise a few long gentle
        /// beauty shots with plain cuts.</summary>
        public ShotSequence PlanAmbient(
            WorldSubjects world,
            ContentIndexSnapshot index,
            MarketingCaptureRecipe recipe,
            int seed)
        {
            ...
            if (recipe.shotCount <= 1)
            {
                var subject = AmbientAnchor(world, index, usage, rng);
                // Vista framing: a lone tile/building (radius ~2-4) reads as a
                // dot from a high orbit; ~14-18 keeps the map filling frame.
                subject.approximateRadius = Mathf.Clamp(subject.approximateRadius, 14f, 18f);
                sequence.shots.Add(new ShotPlan
                {
                    shotId = "loop-01",
                    category = ShotCategory.WorldBeauty,
                    rig = CameraRigType.TacticalOverview,
                    motion = ShotMotion.FullOrbit,
                    transitionIn = ShotTransition.Cut,
            ...
            int count = Mathf.Max(2, recipe.shotCount);
            float per = recipe.durationSec / count;
            var cats = new[] { ShotCategory.Settlement, ShotCategory.WorldBeauty, ShotCategory.Atmosphere };
            var rigs = new[] { CameraRigType.WorldReveal, CameraRigType.Orbit, CameraRigType.SettlementWide, CameraRigType.Dolly };
            CameraRigType lastRig = (CameraRigType)(-1);
            ShotSubject first = null;
            for (int i = 0; i < count; i++)
            {
                var cat = cats[i % cats.Length];
                var rig = rigs[rng.Next(0, rigs.Length)];
                if (rig == lastRig) rig = rigs[(Array.IndexOf(rigs, rig) + 1) % rigs.Length];
                lastRig = rig;
                var shot = new ShotPlan
                {
                    shotId = $"ambient-{i + 1:00}",
                    category = cat,
                    rig = rig,
                    motion = DefaultMotion(rig),
                    transitionIn = i % 4 == 3 ? ShotTransition.Dissolve : ShotTransition.Cut,
                    durationSec = per,
                    motionSpeed = 0.35f,
                    seed = rng.Next(1, int.MaxValue),
                    aspectIntent = AspectOf(recipe),
                    subject = PickSubject(world, index, cat, usage, rng, closeUp: false),
                };
                // Rhyme the wrap: closing shot revisits the opening subject.
                if (i == count - 1 && first != null) shot.subject = first;
                if (i == 0) first = shot.subject;
                sequence.shots.Add(shot);
            }
            return sequence;
        }

        /// <summary>Anchor for the ambient loop: a staged settlement when
        /// present, else the landmark nearest world centre — keeps the orbit
        /// over populated terrain instead of map-edge water — else the world
        /// centre itself.</summary>
        private static ShotSubject AmbientAnchor(
            WorldSubjects world, ContentIndexSnapshot index,
            Dictionary<string, int> usage, MarketingRng rng)
        {
            var s = PickSubject(world, index, ShotCategory.Settlement, usage, rng, closeUp: false);
            if (s.contentId != "world" || world == null) return s;
            ShotSubject best = null;
            float bestD = float.MaxValue;
            foreach (var l in world.landmarks)
            {
                float dx = l.worldX - world.worldCenterX, dz = l.worldZ - world.worldCenterZ;
                float d = dx * dx + dz * dz;
