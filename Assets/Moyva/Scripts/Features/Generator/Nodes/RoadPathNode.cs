            ...
            for (int x = 0; x < w; x++)
            {
                for (int y = 0; y < h; y++)
                {
                    ...
                    if (x > 0 && x < w - 1 && y > 0 && y < h - 1)
                    {
                        ...
                        slope += Mathf.Abs(heightMap[x, y] - heightMap[x, y + 1]);
                        base_cost += slope * _hillPenalty;
                    }

                    if (waterMask != null && waterMask[x, y])
                        base_cost += _waterPenalty;
