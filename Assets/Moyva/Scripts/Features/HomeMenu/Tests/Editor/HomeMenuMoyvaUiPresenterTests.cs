...
{
    /// <summary>
    /// Regression coverage for the dead Controls settings tab. A deferred route
    /// swap that lands on the regional-update path must resync the rendered-route
    /// cache and replay the enter motion — a finished exit fade leaves the
    /// navigation region at alpha 0 (blank panel) and a stale route cache re-arms
    /// the exit fade on every later same-route change such as settings tab
    /// switches. Switching into Controls additionally changes the shell root
    /// class (controls-page), which a regional update cannot retarget, so it must
    /// take the full document mount for the scoped CSS to apply.
    /// </summary>
    public sealed class HomeMenuMoyvaUiPresenterTests
    {
        ...
        public IEnumerator DeferredRouteSwap_ResyncsRenderedRoute_AndControlsTabMounts()
        {
            ...
            try
            {
                ...
                Assert.AreEqual(2, host.MountCalls,
                    "Route swap changes the root class (route-main -> route-settings), so the deferred swap must take the full mount path.");
                Assert.AreEqual(2, CountPlays(host, "slide-up"),
                    "The enter motion must replay after the deferred swap so the region fades back in (initial mount + route enter).");

                // A same-route section switch stays on the regional update path
                // and must not re-arm the exit fade (stale rendered-route cache).
