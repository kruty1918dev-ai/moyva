...
namespace Kruty1918.Moyva.HomeMenu.Runtime
{
    internal static class HomeMenuControlsMarkup
    {
        ...
        public static void Append(StringBuilder sb, HomeMenuMoyvaUiViewController view)
        {
            ...
            sb.Append("<view className=\"controls-intro\"><view className=\"controls-intro-copy\"><text className=\"section-label\">").Append(view.T("CONTROLS")).Append("</text><text className=\"controls-title\">").Append(view.T("Make yourself comfortable.")).Append("</text></view><text className=\"controls-badge\">")
                .Append(E(view.T(HomeMenuControlsEditor.ProfileLabel(editor.ActiveProfile)))).Append("</text></view>");
            sb.Append("<view className=\"device-toolbar\"><text>").Append(view.T("Input profile")).Append("</text>");
            sb.Append("<select className=\"menu-select device-profile-select\" options=\"").Append(E(view.TOptions("Auto|Keyboard + Mouse|Keyboard + Touchpad|Gamepad|Touch / Phone"))).Append("\" value=\"")
                .Append((int)editor.ProfileSelection).Append("\" onChange=\"Globals.moyvaMenu.ControlProfileSelection(event)\"></select>");
            string[] tabs = { view.T("Keyboard"), view.T("Mouse"), view.T("Touchpad"), view.T("Gamepad"), view.T("Phone") };
            for (int i = 0; i < tabs.Length; i++) Choice(sb, tabs[i], "SelectControlDevice(" + i + ")", editor.DeviceTab == i);
            Choice(sb, editor.IsTesting ? view.T("Exit test") : view.T("Test Controls"), "TestControls()", editor.IsTesting);
            Choice(sb, view.T("Reset this profile"), "ResetControlProfile()", false);
            sb.Append("</view>");
            sb.Append("<text className=\"keyboard-hint\">").Append(E(editor.IsTesting
                ? editor.TestReadout
                : view.T("Pick a camera action, then choose or record a shortcut for it."))).Append("</text>");

            // Primary task first: the camera-action list drives the editor below.
            sb.Append("<view className=\"controls-section-heading\"><text className=\"section-label\">").Append(view.T("CAMERA BINDINGS")).Append("</text><text className=\"control-help\">").Append(view.T("Select an action to see and edit its shortcut")).Append("</text></view><view className=\"binding-cards\">");
            foreach (PlayerControlAction action in Enum.GetValues(typeof(PlayerControlAction)))
            {
                ...
                var label = PlayerControlBinding.TryParse(path, out var binding) ? binding.DisplayName : view.T("UNBOUND");
                ...
                sb.Append("\" onClick=\"Globals.moyvaMenu.EditControlAction(").Append((int)action).Append(")\"><text className=\"binding-card-title\">")
                    .Append(E(view.T(HomeMenuControlsEditor.ActionLabel(action)))).Append("</text><text className=\"binding-card-key\">").Append(E(label)).Append("</text></button>");
            }
            sb.Append("</view>");

            // Secondary: the device-specific editor with the inspector alongside.
            sb.Append("<view className=\"controls-section-heading\"><text className=\"section-label\">").Append(view.T("DEVICE LAYOUT")).Append("</text><text className=\"control-help\">").Append(view.T("Pick a key or control on the diagram to inspect it")).Append("</text></view>");
            sb.Append("<view className=\"controls-workspace adaptive-workspace\"><view className=\"keyboard-panel\">");
            if (editor.DeviceTab < 3)
            {
                sb.Append("<view className=\"device-toolbar\"><text>").Append(view.T("Generic pointer")).Append("</text>");
                Choice(sb, view.T("Auto"), "SelectControlPointerMode(0)", editor.PointerMode == PointerInterpretation.Auto);
                Choice(sb, view.T("Use as Mouse"), "SelectControlPointerMode(1)", editor.PointerMode == PointerInterpretation.Mouse);
                Choice(sb, view.T("Use as Touchpad"), "SelectControlPointerMode(2)", editor.PointerMode == PointerInterpretation.Touchpad);
                sb.Append("</view>");
            }
            ...
            if (editor.DeviceTab < 3)
            {
                ...
                foreach (var row in Rows)
                {
                    ...
                    foreach (var key in row.Split(' ')) AppendKey(sb, editor, key, view);
                    sb.Append("</view>");
                }
                sb.Append("</view>");
            }
            AppendDevice(sb, editor, view);
            if (editor.DeviceTab < 2) sb.Append("</view>");
            AppendDeviceOptions(sb, editor, view);
            sb.Append("</view>");
            AppendInspector(sb, view);
            sb.Append("</view><view className=\"controls-section-heading\"><text className=\"section-label\">").Append(view.T("CAMERA FEEL")).Append("</text><text className=\"control-help\">").Append(view.T("Fine-tune movement and sensitivity")).Append("</text></view>");
        }
        ...
        private static void AppendDevice(StringBuilder sb, HomeMenuControlsEditor editor, HomeMenuMoyvaUiViewController view)
        {
            if (editor.DeviceTab < 2)
            {
                sb.Append("<view className=\"device-diagram mouse-diagram\"><text>").Append(view.T("MOUSE")).Append("</text><view className=\"device-toolbar\">");
                ...
                sb.Append("</view><view className=\"device-toolbar\">");
                DeviceControl(sb, editor, "<Mouse>/backButton", view.T("Back")); DeviceControl(sb, editor, "<Mouse>/forwardButton", view.T("Next"));
                sb.Append("</view></view>");
            }
            else if (editor.DeviceTab == 2)
            {
                sb.Append("<view className=\"device-diagram touchpad-diagram\"><text>").Append(view.T("TOUCHPAD · Pointer / tap to select")).Append("</text><view className=\"device-toolbar\">");
                DeviceControl(sb, editor, "<Touchpad>/spaceDrag", view.T("Space + drag ↔"));
                DeviceControl(sb, editor, "<Touchpad>/scroll/up", view.T("Two fingers ↑"));
                DeviceControl(sb, editor, "<Touchpad>/scroll/down", view.T("Two fingers ↓"));
                sb.Append("</view><text>").Append(view.T("Alt + Space + drag to orbit · WASD fallback")).Append("</text></view>");
            }
            else if (editor.DeviceTab == 3)
            {
                ...
                DeviceControl(sb, editor, "<Gamepad>/select", view.T("View")); DeviceControl(sb, editor, "<Gamepad>/start", view.T("Menu"));
                DeviceControl(sb, editor, "<Gamepad>/rightShoulder", "RB"); DeviceControl(sb, editor, "<Gamepad>/rightTrigger", "RT");
                sb.Append("</view><view className=\"device-toolbar\">");
                Stick(sb, editor, "leftStick", view.T("LEFT STICK"));
                ...
                Stick(sb, editor, "dpad", view.T("D-PAD")); Stick(sb, editor, "rightStick", view.T("RIGHT STICK"));
                sb.Append("</view></view>");
            }
            else
            {
                sb.Append("<view className=\"device-diagram phone-diagram\"><view className=\"phone-speaker\"></view><text>").Append(view.T("PHONE · Tap to select")).Append("</text>");
                DeviceControl(sb, editor, "<Touch>/tap", view.T("Tap"));
                DeviceControl(sb, editor, "<Touch>/longPress", view.T("Hold"));
                DeviceControl(sb, editor, "<Touch>/drag", view.T("One finger · Drag ↔"));
                DeviceControl(sb, editor, "<Touch>/pinchOut", view.T("Two fingers · Spread ↗ ↙"));
                DeviceControl(sb, editor, "<Touch>/pinchIn", view.T("Two fingers · Pinch ↘ ↖"));
                DeviceControl(sb, editor, "<Touch>/twist", view.T("Two fingers · Twist ⟳"));
                sb.Append("<text>").Append(view.T("Gestures start outside interface controls")).Append("</text></view>");
            }
        }

        private static void AppendDeviceOptions(StringBuilder sb, HomeMenuControlsEditor editor, HomeMenuMoyvaUiViewController view)
        {
            ...
            if (editor.DeviceTab != 3)
            {
            Choice(sb, view.T("Pan gesture"), "SelectControlGesture(1)", editor.GestureSlot == 1);
            Choice(sb, view.T("Orbit gesture"), "SelectControlGesture(2)", editor.GestureSlot == 2);
            }
            if (editor.DeviceTab < 3) Choice(sb, view.T("Edge pan") + ": " + view.T(options.EdgePan ? "On" : "Off"), "ControlOption(2,0)", options.EdgePan);
            if (editor.DeviceTab == 2 || editor.DeviceTab == 4) Choice(sb, view.T("Gestures") + ": " + view.T(options.Gestures ? "On" : "Off"), "ControlOption(3,0)", options.Gestures);
            sb.Append("</view><view className=\"device-toolbar\"><text>").Append(view.T("Sensitivity")).Append(' ').Append(options.Sensitivity.ToString("0.00", System.Globalization.CultureInfo.InvariantCulture)).Append("</text>");
            Choice(sb, "−", "ControlOption(0," + (options.Sensitivity - 0.25f).ToString(System.Globalization.CultureInfo.InvariantCulture) + ")", false);
            Choice(sb, "+", "ControlOption(0," + (options.Sensitivity + 0.25f).ToString(System.Globalization.CultureInfo.InvariantCulture) + ")", false);
            if (editor.DeviceTab == 3)
            {
                sb.Append("<text>").Append(view.T("Deadzone")).Append(' ').Append(options.Deadzone.ToString("0.00", System.Globalization.CultureInfo.InvariantCulture)).Append("</text>");
        ...
        private static void AppendKey(StringBuilder sb, HomeMenuControlsEditor editor, string definition, HomeMenuMoyvaUiViewController view)
        {
            ...
            var hint = modifier != PlayerControlModifiers.None ? view.TF("Toggle {0} layer", label) : action.HasValue ? view.T(HomeMenuControlsEditor.ActionLabel(action.Value)) : reserved.Length > 0 ? view.T(reserved) : view.T("No binding on this layer");
        ...
        private static void AppendInspector(StringBuilder sb, HomeMenuMoyvaUiViewController view)
        {
            ...
            sb.Append("<view className=\"key-inspector\"><text className=\"section-label\">").Append(view.T("SELECTED SHORTCUT")).Append("</text><text className=\"selected-key-display\">")
                .Append(E(editor.DraftLabel)).Append("</text><text className=\"selected-key-assignment\">")
                .Append(E(bound.HasValue ? view.T(HomeMenuControlsEditor.ActionLabel(bound.Value)) : reserved.Length > 0 ? view.TF("{0} (gameplay)", view.T(reserved)) : view.T("Available for a camera action")))
            ...
            sb.Append("</view><text className=\"inspector-label\">").Append(view.T("ASSIGN TO")).Append("</text><select className=\"menu-select inspector-select\" options=\"");
            foreach (PlayerControlAction action in Enum.GetValues(typeof(PlayerControlAction)))
            {
                if ((int)action > 0) sb.Append('|');
                sb.Append(E(view.T(HomeMenuControlsEditor.ActionLabel(action))));
            }
            sb.Append("\" value=\"").Append((int)editor.SelectedAction).Append("\" onChange=\"Globals.moyvaMenu.SelectControlAction(event)\"></select>");
            ActionButton(sb, editor.IsCapturing ? view.T("CANCEL RECORDING") : view.T("RECORD SHORTCUT"), editor.IsCapturing ? "CancelControlCapture()" : "RecordControlShortcut()", "record-shortcut", view.SettingsInteractable);
            ActionButton(sb, view.T("APPLY SHORTCUT"), "ApplyControlShortcut()", "apply-shortcut", editor.CanApply);
