...
namespace Kruty1918.Moyva.Construction.Editor
{
    public sealed class BuildingDesignerWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            float listWidth = Mathf.Clamp(position.width * 0.24f, 280f, 390f);
            float rightWidth = Mathf.Clamp(position.width * 0.25f, 320f, 470f);

            using (new EditorGUILayout.HorizontalScope(GUILayout.ExpandHeight(true)))
            {
                _perfProfiler.BeginSection("List");
                DrawBuildingList(GUILayout.Width(listWidth), GUILayout.ExpandHeight(true));
                _perfProfiler.EndSection("List");
                _perfProfiler.BeginSection("Main");
                DrawMainPanel(GUILayout.MinWidth(440f), GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
                _perfProfiler.EndSection("Main");
                _perfProfiler.BeginSection("Right");
                DrawRightPanel(GUILayout.Width(rightWidth), GUILayout.ExpandHeight(true));
        ...
        private void DrawToolbar()
        {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                _diffBeforeSaveEnabled = GUILayout.Toggle(
                    ...
                    EditorTooltipStandard.Content("Diff Save", "Показує diff полів перед Save реєстру.", "Зменшує ризик випадково зберегти некоректні зміни будівель."),
        ...
        private void DrawCreateBox(SerializedProperty buildingsProperty)
        {
            using (new EditorGUILayout.VerticalScope(CardStyle()))
            {
                ...
                {
                    EditorWindowSharedUI.DrawWarning(validation, MessageType.Warning);
                    if (GUILayout.Button(
                            EditorTooltipStandard.Content("Quick Fix ID", "Автоматично виправляє ID для нового запису.", "Зменшує час на ручне усунення конфліктів ID перед створенням будівлі."),
                            GUILayout.Height(22f)))
                    {
                        _newBuildingId = GenerateUniqueId(_newBuildingId, buildingsProperty);
                    }
                }
        ...
        private void DrawBuildingListRow(SerializedProperty buildingProperty, int index)
        {
            ...
            Rect row = GUILayoutUtility.GetRect(0f, 58f, GUILayout.ExpandWidth(true));
            ...
            float badgeX = row.xMax - 166f;
            Rect titleRect = new Rect(iconRect.xMax + 8f, row.y + 7f, Mathf.Max(80f, badgeX - iconRect.xMax - 16f), 20f);
            GUI.Label(titleRect, string.IsNullOrWhiteSpace(displayName) ? id : displayName, EditorStyles.boldLabel);
            GUI.Label(new Rect(titleRect.x, titleRect.yMax + 1f, titleRect.width, 16f), id, EditorStyles.miniLabel);

            DrawBadgeRect(new Rect(badgeX, row.y + 9f, 86f, 18f), category.ToString(), CategoryColor(category));
            DrawBadgeRect(new Rect(badgeX + 90f, row.y + 9f, 68f, 18f), placement.IsAllowed ? "OK" : "BLOCK", placement.IsAllowed ? AllowedColor : BlockedColor);
            if (hasErrors)
                DrawBadgeRect(new Rect(badgeX + 90f, row.y + 31f, 68f, 18f), "ERR", BlockedColor);
        ...
        private void DrawUsageGuide()
        {
            using (new EditorGUILayout.VerticalScope(CardStyle()))
            {
                ...
                EditorGUILayout.LabelField("1. Оберіть будівлю кліком по запису зліва.", EditorStyles.wordWrappedMiniLabel);
                EditorGUILayout.LabelField("2. Перемикайте Будівля / Симуляція / Гравець у верхній панелі.", EditorStyles.wordWrappedMiniLabel);
                EditorGUILayout.LabelField("3. У Симуляції клікайте по мапі: режим Тест ставить позицію, інші режими малюють сценарій.", EditorStyles.wordWrappedMiniLabel);
                EditorGUILayout.LabelField("4. Preview бере SpriteRenderer з prefab; Icon використовується як fallback.", EditorStyles.wordWrappedMiniLabel);
            }
        }
        ...
        private void DrawBuildingFacts(BuildingDefinition definition)
        {
            ...
            using (new EditorGUILayout.VerticalScope(CardStyle()))
            {
                ...
                {
                    if (GUILayout.Button(
                            EditorTooltipStandard.Content("Quick Fix Typical", "Виправляє типові проблеми вибраної будівлі: ID, назву, prefab, icon.", "Дає швидкий старт до валідного стану без ручного проходу по полях."),
                            GUILayout.Height(22f)))
                    {
                        QuickFixSelectedBuildingTypicalIssues();
                    }
