...
namespace Kruty1918.Moyva.Units.Editor
{
    public sealed partial class UnitDesignerWindow
    {
        ...
        {
            BeginSection("Розрахунок захисту", "d_Profiler.Memory", "Кожен захист поглинає відповідний тип шкоди перед сумуванням.");
        ...
        {
            BeginSection("Підсумок дуелі", "d_Animation.EventMarker", "Скільки ударів потрібно кожному юніту, щоб перемогти іншого.");
        ...
        private void RequestApplyAllCombatPresets()
        {
            if (!_safeEditMode)
            {
            ...
            int createCount = 0;
            int updateCount = 0;
            var previewLines = new List<string>(CombatTablePresets.Length);
            ...
            for (int i = 0; i < CombatTablePresets.Length; i++)
            {
                ...
                previewLines.Add($"{(created ? "Створити" : "Оновити")} {preset.TypeId}: HP {preset.HitPoints}, огляд {preset.BaseViewDistance}, {preset.CombatType}");
            }

            QueueSafeEditOperation(
                "Масове застосування combat presets",
                $"Буде створено: {createCount}, оновлено: {updateCount}.",
                previewLines,
                ApplyAllCombatPresets);
        }
        ...
        private void ApplyAllCombatPresets()
        {
            ...
            for (int i = 0; i < CombatTablePresets.Length; i++)
            {
                ...
                ApplyCombatPreset(_configs.GetArrayElementAtIndex(index), preset, applyIdentity: true, clearPrefab: created, commitChanges: false);
            }

            if (TryCommitRegistryChanges("Масове застосування combat presets"))
                SaveSelectedPreference();
        }

        private void ApplyCombatPreset(SerializedProperty unit, UnitCombatPreset preset, bool applyIdentity, bool clearPrefab, bool commitChanges = true)
        {
            if (unit == null)
                return;
            ...
            if (commitChanges)
                TryCommitRegistryChanges("Застосування combat preset");
        }
