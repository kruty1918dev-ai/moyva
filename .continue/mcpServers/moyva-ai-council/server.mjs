...
async function loadMoyvaLocalEnv() {
  ...
  try {
    ...
    if (error?.code !== "ENOENT") {
      console.error(
        `[Moyva Council] env load failed: ${error.message}`
      );
    }
  }
}

await loadMoyvaLocalEnv();
...
const PROVIDER =
  (process.env.COUNCIL_PROVIDER || "ollama").toLowerCase();
...
const OPENROUTER_MODELS = {
  scout:
    process.env.COUNCIL_OR_SCOUT ||
    "openrouter/free",

  reasoner:
    process.env.COUNCIL_OR_REASONER ||
    "openrouter/free",

  engineer:
    process.env.COUNCIL_OR_ENGINEER ||
    "openrouter/free",

  architect:
    process.env.COUNCIL_OR_ARCHITECT ||
    "openrouter/free"
};
...
async function openRouterRequest({
  ...
  try {
    console.error(
      `[Moyva Council] OpenRouter → ${model}`
    );
...
async function modelRequest(args) {
  if (PROVIDER === "openrouter") {
    return openRouterRequest({
      ...args,

      model:
        OPENROUTER_MODELS[args.role] ||
        "openrouter/free"
    });
  }

  return ollamaRequest(args);
}
