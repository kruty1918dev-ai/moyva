    ...
    internal sealed class RecruitmentService : IRecruitmentService, ITurnParticipant, ISaveModule
    {
        ...
        public void OnTurnStarted(TurnContext context)
        {
            foreach (QueueState queue in _queues.Values)
            {
                ...
                if (job.Status == RecruitmentJobStatus.ReadyToDeploy && TryFindSpawn(queue.Position, ResolveModule(queue.BuildingId)?.SpawnRadius ?? 2, out Vector2Int spawn))
                {
                    string unitId = _units.CreateUnit(job.UnitTypeId, spawn, queue.OwnerId);
