...
| Camera zoom state (shared normalized zoom → far-view weight) | `Camera/API/ICameraZoomState.cs`; `Camera/Runtime/CameraZoomStateService.cs`; window in `Presets/Systems/camera-settings` (`farView`) |
| Far-view atmosphere (high-altitude grade + cloud veil) | `Visuals/Runtime/FarView/FarViewAtmosphereRendererFeature.cs`; `Visuals/Runtime/FarView/FarViewAtmosphereDriver.cs`; `Art/Shaders/URP/FarViewAtmosphere.shader`; preset `Presets/Systems/far-view-atmosphere` |
