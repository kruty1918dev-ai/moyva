...
Construction/
└── UI/
    ├── Kruty1918.Moyva.Construction.UI.asmdef     ← збірка: залежить від Construction + Signals + TextMeshPro
    │
    ├── ConstructionUIController.cs     ← головний адаптер UI ↔ IConstructionService
    ├── BuildingMenuFactory.cs          ← формування меню з реєстру (enum-категорії + іконки)
...
**Залежності збірки:**
`Kruty1918.Moyva.Construction` · `Kruty1918.Moyva.Signals` · `Zenject` · `Unity.TextMeshPro`

Запити на зміну режиму гри виконуються через сигнал `GameModeChangeRequestedSignal`,
який обробляє `GameModeChangeRequestRouter` у модулі GameMode.
...
### Швидкий старт — автоматичне створення (рекомендовано)

Відкрий **Moyva → Construction → UI Setup Tool** у меню Unity.

Натисни **«Створити усю UI-ієрархію у сцені»** — інструмент автоматично:

| Що створює | Результат |
|---|---|
| `Canvas` + `EventSystem` | тільки якщо відсутні у сцені |
| Canvas Scaler | `Scale With Screen Size` (1920×1080, Match 0.5) для гарного вигляду на різних екранах |
| `ConstructionUI` (root) | `ConstructionUIController` + `ConstructionUIInstaller` |
| `StatusPanel` | верхня панель на всю ширину (`ConstructionStatusUI` + 3 статус-мітки) |
| `BuildingSelectionPanel` | нижня панель зі **ScrollRect по горизонталі** + `Content` (HorizontalLayout) |
| `CategoryTabs` | ряд класів будівель усередині нижньої панелі (`BuildingCategoryTabsUI`) |
| `ActionBar` | `ConstructionActionBarUI` + 5 кнопок (Confirm/Cancel/Undo/Redo/Знести) |
| `Templates` | auto-created шаблони `tabButtonPrefab` та `buttonPrefab` (призначаються автоматично) |

Всі серіалізовані посилання між компонентами призначаються автоматично.
Залишається лише **призначити `gameplayUIRoot`** у Inspector контролера (кореневий GameObject основного UI).

> Операція повністю undo-able через Ctrl+Z.

---

## DOTween для анімацій натиску

Усі кнопки Construction UI (`BuildingButtonUI`, `BuildingCategoryTabsUI`, `ConstructionActionBarUI`)
мають серіалізовані параметри анімації натиску (`pressScaleMultiplier`, `pressDuration`, `pressVibrato`, `pressElasticity`).

Для активації анімацій:
1. Імпортуй DOTween у проєкт (через Package Manager/Git URL/OpenUPM або .unitypackage).
2. Після появи пакета `com.demigiant.dotween` Unity автоматично застосує define `DOTWEEN_ENABLED` для UI asmdef.
3. Налаштуй параметри анімації в Inspector для кожного UI-компонента.

Примітка: у базовому `Packages/manifest.json` пакет DOTween не зафіксований,
тому за його відсутності система працює без анімацій натиску (без compile-помилок).
...
│
└── Templates (inactive)
    ├── CategoryTabButtonTemplate
    └── BuildingButtonTemplate
```
...
2. Перетягни в Inspector:
    - `itemContainer` → Transform з **HorizontalLayoutGroup** (контент горизонтального скролу)
...
> В автосетапі ці посилання вже призначаються автоматично, включно з `ScrollRect` і шаблоном кнопки.
