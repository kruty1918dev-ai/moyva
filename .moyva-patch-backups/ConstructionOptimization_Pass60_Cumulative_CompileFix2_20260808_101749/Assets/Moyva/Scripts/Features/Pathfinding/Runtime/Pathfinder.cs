...
{
	public sealed class Pathfinder : IOccupiedCellPathfinder
	{
		...
		private readonly List<Vector2Int> _neighborScratch =
			...
			Func<Vector2Int, bool> canTraverseOccupied)
		{
			...
			// Occupied target is valid only when the caller explicitly allows it.
			if (_objectsMapService.IsOccupied(end)
				&& (canTraverseOccupied == null
					|| !canTraverseOccupied(end)))
			{
				return new List<Vector2Int>();
			}
			...
			PushOpenNode(
				new OpenNode(
					start,
					0f,
					_neighborhoodStrategy.EstimateDistance(start, end)));
			...
			Func<Vector2Int, bool> canTraverseOccupied)
		{
			...
			float tentativeG =
				bestKnownG
				+ _neighborhoodStrategy.GetStepCost(
					current,
					neighbor)
				* tileWeight;
			...
			float f =
				tentativeG
				+ _neighborhoodStrategy.EstimateDistance(
					neighbor,
					end);
			PushOpenNode(
				new OpenNode(
		...
		private static bool IsOpenNodeBetter(
			OpenNode left,
			OpenNode right)
		{
			if (left.F < right.F)
