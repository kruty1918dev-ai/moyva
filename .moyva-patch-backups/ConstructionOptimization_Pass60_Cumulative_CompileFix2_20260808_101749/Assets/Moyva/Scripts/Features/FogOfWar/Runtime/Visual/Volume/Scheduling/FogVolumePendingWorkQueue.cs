...
namespace Kruty1918.Moyva.FogOfWar.Runtime
{
    ...
    internal sealed class FogVolumePendingWorkQueue : IFogVolumePendingWorkState, IFogVolumePendingWorkRequests, IFogVolumePendingWorkMaintenance
    {
            ...
            IReadOnlyList<FogCellVisualChange> changes)
        {
            ...
            if (changes != null)
            {
                for (int i = 0; i < changes.Count; i++)
                {
                    ...
                    if (_pendingCellChangeIndexByCell.TryGetValue(
                            change.Cell,
                            out int existingIndex))
                    {
                        FogCellVisualChange existing =
                            _pendingCellChanges[existingIndex];
                        _pendingCellChanges[existingIndex] =
                            new FogCellVisualChange(
                                change.Cell,
                                existing.OldState,
                                change.NewState,
                                existing.OldHeightKey,
                                change.NewHeightKey);
                        continue;
                    }

                    _pendingCellChangeIndexByCell[change.Cell] =
                        _pendingCellChanges.Count;
        ...
        public void Complete()
        {
            ...
            _pendingCellChangeIndexByCell.Clear();
