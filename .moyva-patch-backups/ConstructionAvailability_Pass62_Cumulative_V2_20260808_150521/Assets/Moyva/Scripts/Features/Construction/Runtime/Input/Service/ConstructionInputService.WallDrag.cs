...
namespace Kruty1918.Moyva.Construction.Runtime
{
    internal sealed partial class ConstructionInputService
    {
        ...
        private void BeginWallUndoBatch()
        {
            if (_wallUndoBatchActive)
                return;

            if (_constructionService
                is not IConstructionPendingUndoBatch batch)
            {
                return;
            }
        ...
        private void EndWallUndoBatch()
        {
            ...
            if (_constructionService
                is IConstructionPendingUndoBatch batch)
            {
                batch.EndPendingUndoBatch();
            }
