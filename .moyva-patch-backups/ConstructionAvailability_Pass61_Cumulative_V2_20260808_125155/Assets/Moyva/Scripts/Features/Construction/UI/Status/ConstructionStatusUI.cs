...
namespace Kruty1918.Moyva.Construction.UI
{
    ...
    public class ConstructionStatusUI : MonoBehaviour
    {
        ...
        private bool _hasLastUiState;
        private int _lastPlacementStateValue;
        private int _lastPreviewStateValue;
        ...
        {
            int placementStateValue =
                (int)state.PlacementState;
            if (!_hasLastUiState
                || _lastPlacementStateValue
                    != placementStateValue)
            {
                _lastPlacementStateValue =
                    placementStateValue;
                if (placementStateLabel != null)
                {
                    placementStateLabel.text =
                        state.PlacementState.ToString();
                }
            }
            ...
            int previewStateValue =
                (int)state.LastPreviewState;
            if (!_hasLastUiState
                || _lastPreviewStateValue
                    != previewStateValue)
            {
                _lastPreviewStateValue =
                    previewStateValue;
